...
public class Network extends DirectedGraph {
	 ...
	 */
        ...
        public boolean checkCapacity() {
            int vertMax = getNumVertices();
            Vertex v1;
            for(int i = 0; i < vertMax; i++) {
                v1 = getVertexByIndex(i);
                for(Vertex v2: getAdjList(v1)) {
                    Edge e = getAdjMatrixEntry(v1, v2);
                    if (e.getFlow() > e.getCap()) return false;
                }
            }
            return true;
        }

        public boolean checkFlowInvariant() {
            int in;
            int out;
            for (int i = 0; i < getNumVertices(); i++) {
                ...
                for (int j = 0; j < getNumVertices(); j++) {
                   in += adjMatrix[i][j].getFlow();
                   out += adjMatrix[j][i].getFlow();
                }
	...
	public boolean isFlow() {
                boolean cap = checkCapacity();
                if (!cap) return false;
                boolean flo = checkFlowInvariant();
                if (!flo) return false;
		return true;
	}
